...
void decrease_month(int *month,  int *year){ //decrease the month by one
    --*month;
...
int checkNote(int date, int month)
{
    FILE *fp;
...
void showNote(int month)
{
	...
	while(fread(&R,sizeof(R),1,fp) == 1)
	{
        if(R.month == month){
            gotoxy(10,5+i);
            printf("Note %d Day = %d: %s", i+1, R.date,  R.note);
            isFound = 1;
            i++;
        }
    }
    ...
    while(1)
	{
        ...
        switch(choice)
		{
            case 1:
                ...
                printf("Day is : %s",getDay(date.date,date.month,date.year));
